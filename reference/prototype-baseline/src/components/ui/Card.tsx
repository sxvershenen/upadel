...
export function SurfaceCard({ tone = "white", interactive = true, className, children, ...props }: SurfaceCardProps) {
  ...
  const toneClass = tone === "white" ? "bg-white text-ink shadow-card" : tone === "glass" ? "glass text-white" : cn("relative isolate overflow-hidden", meshMap[tone as MeshTone], meshTextTone[tone as MeshTone]);
  return <motion.div
