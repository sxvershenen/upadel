...
import { blogMain, blogSecondary, coaches, reviews, tournaments } from "../data/content";
import {
  Accordion,
  ArticleCard,
  ArticleRow,
...
function Demo({ title, children, dark = false }: { title: string; children: ReactNode; dark?: boolean }) {
  return <div className={dark ? "se-3 mesh-dark p-5 text-white md:p-7" : "se-3 border border-ink/10 bg-white p-5 md:p-7"}>
  ...
  return <div className="min-h-screen overflow-x-hidden bg-page text-ink">
    <header className="sticky top-0 z-50 border-b border-white/10 bg-ink/95 text-white backdrop-blur-xl">
      <div className="container-page flex min-h-[72px] items-center justify-between gap-4 py-3">
        ...
        <ButtonLink href={window.location.pathname} variant="glass" size="sm" icon={<ArrowLeft size={16} />} iconPosition="left" iconDivider={false}>На лендинг</ButtonLink>
      </div>
    </header>
    ...
    <main className="container-page">
      ...
      <KitSection id="foundation" eyebrow="Foundation" title="Цвет, типографика, пространство" note="Выбирайте семантическую роль. Числовая шкала и responsive-переходы принадлежат CSS, а не разметке.">
        <div className="grid gap-4 xl:grid-cols-2">
          <Demo title="Semantic & artistic colors"><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{colorTokens.map(([label, className]) => <div key={label}><div className={`se-2 h-20 border border-ink/10 ${className}`} /><Typography role="caption" className="mt-2 text-ink-soft">{label}</Typography></div>)}</div></Demo>
          ...
          <Demo title="Radii & shadows"><div className="grid grid-cols-3 gap-3"><div><div className="se-1 h-24 bg-control" /><CssTokenValue name="--se-1" label="Control" /></div><div><div className="se-2 h-24 bg-control shadow-card" /><CssTokenValue name="--se-2" label="Action" /></div><div><div className="se-3 h-24 bg-white shadow-card-lg" /><CssTokenValue name="--se-3" label="Card" /></div></div></Demo>
        </div>
      </KitSection>
      ...
      <KitSection id="surfaces" eyebrow="Surfaces" title="Поверхности и художественные слои" note="Mesh и overlays — разрешённые выразительные исключения. Фирменный spring lift включён по умолчанию; interactive=false оставляет служебную поверхность неподвижной.">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          ...
          {overlayTones.map((overlay) => <div key={overlay} className={`se-3 ${overlay} relative isolate min-h-[150px] overflow-hidden bg-cover bg-center`} style={{ backgroundImage: `url(${images.offerEvent})` }}><div className="relative z-10 flex h-full min-h-[150px] items-end p-5 text-white"><Typography role="caption" tone="inverse" className="font-semibold uppercase">{overlay}</Typography></div></div>)}
        </div>
      </KitSection>
      ...
      <KitSection id="patterns" eyebrow="Production cards" title="Точные карточки лендинга" note="Каждый пример ниже импортирован из того же production-компонента и использует те же реальные данные, что соответствующая секция.">
        <div className="grid gap-10">
          <Demo title="Coach · opens accessible dialog"><div className="max-w-[420px]"><CoachCard coach={coaches[0]} /></div></Demo>
          <Demo title="Articles · main, mobile/plain and row"><div className="grid gap-8 md:grid-cols-3"><ArticleCard post={blogMain[0]} /><ArticleCard post={blogMain[1]} mobilePlain /><div><ArticleRow post={blogSecondary[0][0]} /><ArticleRow post={blogSecondary[0][1]} /></div></div></Demo>
          <Demo title="Review"><div className="max-w-[540px]"><ReviewCard review={reviews[0]} /></div></Demo>
          <Demo title="Tournament & training"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3"><TournamentCard tournament={tournaments[0]} /><TournamentCard tournament={tournaments[1]} /><TrainingCard training={trainings[0]} /></div></Demo>
          <Demo title="Rent pricing · exact spacing"><div className="grid gap-4 lg:grid-cols-2"><RentDayCard /><RentPrimeCard /><div className="lg:col-span-2"><RentTrialCard /></div><RentStandardsCard /></div></Demo>
          <Demo title="Memberships"><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4"><GiftMembershipCard /><MembershipSmallCard /><MembershipMediumCard /><MembershipResidentCard /></div></Demo>
          <Demo title="Offers"><div className="grid gap-4 md:grid-cols-2"><OfferCard offer={offers[0]} /><OfferCard offer={offers[1]} /></div></Demo>
          <Demo title="Benefits · full production set"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{benefitCardComponents.map(({ id, Component, wide }) => <div key={id} data-benefit-recipe={id} className={wide ? "min-h-[320px] xl:col-span-2" : "min-h-[320px]"}><Component /></div>)}</div></Demo>
          <Demo title="Gallery"><div data-gallery-recipe><GalleryCard src={images.gallery[0]} /></div></Demo>
          <Demo title="Court glass · full production set" dark><div className="grid gap-4 md:grid-cols-2">{courtCardComponents.map(({ id, Component }) => <div key={id} data-court-recipe={id} className="min-h-[320px]"><Component /></div>)}</div></Demo>
        </div>
      </KitSection>
    </main>

    <footer className="bg-ink py-8 text-white"><div className="container-page flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><Typography role="body-small" className="text-white/55">UI kit использует тот же public API, что и лендинг.</Typography><ButtonLink href={window.location.pathname} variant="glass" size="sm" icon={<ArrowLeft size={16} />} iconPosition="left" iconDivider={false}>Вернуться</ButtonLink></div></footer>
  </div>;
}
