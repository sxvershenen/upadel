...
export function Tournaments() {
  ...
  return <section id="tournaments" className="container-page py-20 md:py-28">
    <Reveal><SectionHeader eyebrow={home.tournamentsSection.eyebrow} title={home.tournamentsSection.title} titleClassName="max-w-[150px] md:max-w-none" action={<div className="flex shrink-0 flex-col items-end gap-2"><ButtonLink href="/tournaments" variant="neutral" size="sm" icon={<ArrowRight size={15} />} className="w-[88px] justify-between">Все</ButtonLink><MobileSwiperNav className="md:hidden" atStart={atStart} atEnd={atEnd} onPrev={() => swiperRef.current?.slidePrev()} onNext={() => swiperRef.current?.slideNext()} /></div>} className="mb-10 !flex-nowrap items-start gap-3" /></Reveal>
