...
export function Footer() {
  ...
  const socialIcons = { telegram: Send, video: Video }
  ...
  return <footer id="footer" className="mesh-dark relative overflow-hidden pb-[calc(96px+env(safe-area-inset-bottom))] pt-20 text-white md:pb-16">
    <div className="container-page"><div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
      <Reveal><div className="flex flex-col gap-6"><div className="se-3 h-[260px] w-full overflow-hidden bg-white/5"><iframe title={`Карта клуба ${site.brandName}`} src={mapURL} className="h-full w-full grayscale" loading="lazy" /></div><div className="grid grid-cols-1 gap-2 sm:grid-cols-2">{contactRows.map((row) => { const Tag = row.href ? 'a' : 'div'; const channel = row.href?.startsWith('tel:') ? 'phone' : row.href?.startsWith('mailto:') ? 'email' : null; return <Tag key={row.label} href={row.href} data-analytics-action={channel ?? (row.href?.startsWith('http') ? 'directions' : undefined)} onClick={channel ? (event) => { event.preventDefault(); requestContact(channel) } : undefined} target={row.href?.startsWith('http') ? '_blank' : undefined} rel={row.href?.startsWith('http') ? 'noreferrer' : undefined} className="se-2 flex items-start gap-3 bg-white/5 p-4 transition-colors hover:bg-white/10"><row.icon size={16} className="mt-0.5 shrink-0 text-white/45" /><span className="flex flex-col gap-0.5"><span className="type-caption text-white/45">{row.label}</span><span className="type-ui font-medium text-white/85">{row.value}</span></span></Tag> })}</div></div></Reveal>
      <Reveal delay={0.08}><div className="flex flex-col gap-6">{footer.image && <div className="se-3 aspect-[16/10] w-full overflow-hidden"><img src={footer.image.url} alt={footer.image.alt} className="h-full w-full object-cover" /></div>}<p className="type-body-sm max-w-[520px] text-white/60">{footer.about}</p><div className="grid grid-cols-4 gap-3 border-t border-white/10 pt-6 text-center sm:text-left">{footer.stats.map(({ value, label }) => <div key={label} className="flex flex-col gap-1"><span className="type-title-card font-semibold text-white">{value}</span><span className="type-micro leading-tight text-white/45">{label}</span></div>)}</div></div></Reveal>
    </div></div>
