...
export function ReviewsFAQ() {
  ...
  const faqItems = entities.faqs.map(({ question, answer }) => ({ q: question, a: answer }));
  ...
  return (
    <section className="container-page py-20 md:py-28">
      <div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
        <div>
          <Reveal>
            <SectionHeader eyebrow={home.reviewsSection.reviewsEyebrow} title={home.reviewsSection.reviewsTitle} className="mb-8" />
          </Reveal>

          <Reveal>
            <a
              href={home.reviewsSection.externalReviewsURL}
              target="_blank"
              rel="noreferrer"
              className="se-3 mb-6 flex items-center justify-between gap-4 bg-white p-5"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="type-ui flex items-center gap-1 font-semibold text-ink">
                    <Star size={13} className="fill-amber-400 text-amber-400" /> {home.reviewsSection.externalRatingLabel}
                  </div>
                  <div className="type-caption text-ink-soft">{home.reviewsSection.externalReviewsLabel}</div>
                </div>
              </div>
