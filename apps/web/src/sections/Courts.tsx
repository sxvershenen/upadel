...
export function Courts() {
  ...
  return <section className="relative overflow-hidden py-24 md:py-32">
    <div ref={backgroundRef} data-parallax-viewport className="parallax-viewport absolute inset-0"><motion.div data-parallax-layer style={{ y: backgroundY }} className="parallax-layer-wide overflow-hidden">{home.courtsSection.background && <ProgressiveImage src={home.courtsSection.background.url} alt={home.courtsSection.backgroundAlt} loading="lazy" decoding="async" className="h-full w-full scale-110 object-cover" />}</motion.div></div>
    ...
    <div className="container-page relative z-10">
      ...
      <div className="grid gap-4 lg:grid-cols-12">{entities.courts.map((court, index) => { const columns = court.cardVariant === "panoramic" ? 7 : court.cardVariant === "metrics" ? 5 : 6; return <Reveal key={court.id} delay={cardDelays[index]} fade={false} className={columnClasses[columns]}><CourtCard court={court} /></Reveal> })}</div>
    </div>
  </section>;
}
