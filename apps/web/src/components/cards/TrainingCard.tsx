...
export function TrainingCard({ training, interactive = true }: { training: Training; interactive?: boolean }) {
  ...
  return <ImageCard reveal={false} interactive={interactive} src={training.image.url} alt={training.image.alt} overlay={training.overlay as ImageOverlay} className="h-[500px] min-h-[500px]"><div className="flex h-full flex-col justify-between p-6 md:p-7"><div className="flex items-start justify-between"><Badge tone="outline-light" icon={<Icon size={13} />}>{training.badge}</Badge></div><div><h3 className="type-title-card text-white">{training.title}</h3><p className="type-body-sm mt-2 text-white">{training.description}</p><div className="mt-6 flex items-end justify-between border-t border-white/15 pt-4"><Price label="час от" value={training.priceFrom} tone="light" /><ArrowAction tone="glass" size="sm" cardHover /></div></div></div></ImageCard>
}
