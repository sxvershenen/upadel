...
function TournamentBody({ tournament }: { tournament: Tournament }) {
  ...
  return <div className="relative flex h-full flex-col p-6 md:p-7"><div className="flex items-start justify-between gap-3"><Badge tone="glass" icon={<Icon size={14} />}>{tournament.formatLabel}</Badge>{completed && <Badge tone="glass" icon={<CheckCircle2 size={14} />}>Завершено</Badge>}</div><div className="mt-auto"><h3 className="type-title-large text-white">{tournament.title}</h3><div className="mt-5 flex flex-col gap-2"><MetaRow icon={<CalendarDays size={14} />} text={tournament.scheduleLabel} /><MetaRow icon={<Gauge size={14} />} text={`Уровень ${tournament.levelLabel}`} /><MetaRow icon={<Wallet size={14} />} text={tournament.entryFee} /></div><p className="type-body-sm mt-5 text-white/80">{tournament.description}</p><div className="mt-5 flex items-end justify-between gap-4 border-t border-white/15 pt-4"><Price label={tournament.prizeLabel} value={tournament.prize} tone="light" /><ArrowAction tone="glass" size="sm" cardHover /></div></div></div>;
}
