...
export function OfferCard({ offer }: { offer: Offer }) {
  ...
  return <ImageCard loading="eager" src={offer.image.url} alt={offer.image.alt} overlay={offer.overlay as import("../ui/Card").ImageOverlay} className="aspect-square md:aspect-[2/1]"><div className="flex h-full flex-col justify-between p-6 md:p-8"><div className="flex items-start justify-between"><Badge tone="outline-light" icon={<Icon size={13} />}>{offer.badge}</Badge><ArrowAction tone="glass" cardHover /></div><div className="max-w-[420px]"><h3 className="type-title-card text-white">{offer.title}</h3><p className="type-body-sm mt-2 text-white/80">{offer.description}</p></div></div></ImageCard>;
}
