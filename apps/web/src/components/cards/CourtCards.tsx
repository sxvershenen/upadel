...
export function CourtCard({ court }: { court: Court }) {
  if (court.cardVariant === 'metrics') return <GlassCard interactive={false} className="grid h-full grid-cols-2 p-2">{(court.metrics ?? []).map((metric, index) => { const Icon = icons[metric.icon]; return <div key={metric.label} className={cn("flex flex-col justify-center gap-1 p-6", index % 2 === 0 && "border-r border-white/10", index < 2 && "border-b border-white/10")}><span className="se-2 mb-3 flex h-8 w-8 items-center justify-center bg-white/10 text-white/70"><Icon size={17} /></span><span className="type-price font-semibold text-white/90">{metric.value}</span><span className="type-caption text-white/50">{metric.label}</span></div> })}</GlassCard>
