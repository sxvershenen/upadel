...
export function ArticleCard({ post, mobilePlain = false }: { post: Article; mobilePlain?: boolean }) {
  ...
  return <motion.a {...revealAttributes(true)} href={`/blog/${post.slug}`} initial="rest" whileHover="hover" variants={{ rest: { y: 0, scale: 1 }, hover: { y: -5, scale: 1.012 } }} transition={springSoft} className="card-spring group group/card flex cursor-pointer flex-col">
...
export function ArticleRow({ post, delay = 0 }: { post: ArticleSummary; delay?: number }) {
  return <motion.a {...revealAttributes({ delay, y: 14 })} href={`/blog/${post.slug}`} initial={false} whileHover="hover" variants={{ hover: { x: 4 } }} transition={springSoft} className="card-spring group group/card flex cursor-pointer items-center justify-between gap-4 border-b border-ink/10 px-3 py-5 first:pt-0">
