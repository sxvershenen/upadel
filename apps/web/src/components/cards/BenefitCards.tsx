...
export function BenefitCard({ benefit, mobile = false }: { benefit: Benefit; mobile?: boolean }) {
  ...
  const internalAction = benefit.action.mode === 'internal-link' && benefit.action.href
    ...
    : null
  return <MeshCard tone={tone} interactive={benefit.variant !== 'parking'} className="relative flex h-full min-h-[320px] p-6 md:p-7">{internalAction && <InternalCardTarget {...internalAction} />}{benefit.media && <ProgressiveImage skeleton={false} media={benefit.media} alt="" aria-hidden="true" loading="eager" fetchPriority="low" decoding="async" variants={decorativeVariants} transition={springSoft} className={`pointer-events-none absolute ${mediaClass} z-0 w-auto opacity-100`} />}<div className="relative z-10 flex h-full w-full flex-col"><div className="flex items-start justify-between gap-4"><span className={`se-2 flex h-12 w-12 shrink-0 items-center justify-center bg-[color-mix(in_oklab,#ffffff_70%,transparent)] ${iconColor}`}><Icon size={22} strokeWidth={1.8} /></span><span className={`type-eyebrow max-w-[70%] text-right ${bodyColor}`}>{benefit.eyebrow}</span></div><div className={benefit.variant === 'kids-wide' ? 'my-8 max-w-[78%]' : 'absolute inset-x-0 bottom-0'}><div className={benefit.variant === 'parking' ? 'max-w-[62%]' : benefit.variant === 'kids-wide' ? 'max-w-[78%]' : 'max-w-[68%]'}><h3 className={`type-title-card ${color}`}>{benefit.title}</h3><p className={`type-body-sm mt-2 ${bodyColor}`}>{benefit.description}</p></div></div>{benefit.variant === 'kids-wide' && <div className="mt-auto flex items-center justify-between border-t border-[#2563a3]/20 pt-4"><span className="type-ui flex max-w-[64%] items-center gap-2 font-semibold text-[#18527c]"><ShieldCheck className={iconColor} size={17} /> {benefit.supportingText}</span></div>}{benefit.variant === 'online-booking' && <span className="absolute bottom-0 right-0"><FeatureArrow color={iconColor} /></span>}{benefit.variant === 'coaches-metric' && <span className="absolute bottom-0 right-0"><FeatureArrow color={iconColor} /></span>}</div></MeshCard>
}
