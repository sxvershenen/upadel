...
export function MembershipCard({ membership }: { membership: Membership }) {
  ...
  const card = <WhiteCard className="flex h-full min-h-[500px] flex-col p-6 md:p-7">{body}</WhiteCard>
  return featured ? <div className="relative h-[500px] min-h-[500px] pt-1"><Badge tone="lime" className="se-full absolute -top-2 left-1/2 z-20 -translate-x-1/2 px-4">Хит сезона</Badge>{card}</div> : <div className="h-[500px] min-h-[500px]">{card}</div>
}
