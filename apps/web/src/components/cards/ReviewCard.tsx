...
export function ReviewCard({ review }: { review: Review }) {
  return <WhiteCard className="flex w-full flex-col gap-4 p-6">
    <div className="flex items-center gap-3">{review.avatar && <img src={review.avatar.url} alt={review.avatar.alt} className="h-11 w-11 rounded-full object-cover" />}<div><div className="type-ui text-ink">{review.authorName}</div><div className="type-caption text-ink-soft">{review.authorMeta}</div></div></div>
