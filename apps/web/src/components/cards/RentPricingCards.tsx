...
export function RentalRateCard({ rate, sourceEntity, sourcePage, layout = 'compact' }: { rate: RentalRate; sourceEntity?: string; sourcePage?: string; layout?: 'wide' | 'compact' }) {
  if (rate.cardVariant === 'trial' && layout === 'wide') {
    return <MeshCard reveal={false} tone={(rate.meshTone ?? 'deep-blue') as MeshTone} className="relative flex h-full min-h-[240px] flex-col overflow-hidden p-7 text-white md:p-8">
      <motion.img src="/padel-racket.webp" alt="" aria-hidden="true" variants={{ rest: { scale: 1, rotate: 7, y: 0 }, hover: { scale: 1.05, rotate: 3, y: -6 } }} transition={springSoft} className="pointer-events-none absolute -bottom-14 right-[8%] z-0 h-[340px] w-auto opacity-95" />
      <motion.img src="/padel-ball.webp" alt="" aria-hidden="true" variants={{ rest: { scale: 1, y: 0 }, hover: { scale: 1.08, y: -5 } }} transition={springSoft} className="pointer-events-none absolute -bottom-8 -right-4 z-0 h-[210px] w-auto opacity-95" />
  ...
  if (rate.cardVariant === 'trial') {
    return <MeshCard reveal={false} tone={(rate.meshTone ?? 'deep-blue') as MeshTone} className="relative flex h-full min-h-[500px] flex-col overflow-hidden p-6 text-white md:p-7">
      <motion.img src="/padel-racket.webp" alt="" aria-hidden="true" variants={{ rest: { scale: 1, rotate: 7, y: 0 }, hover: { scale: 1.05, rotate: 3, y: -6 } }} transition={springSoft} className="pointer-events-none absolute -bottom-16 -right-6 z-0 h-[270px] w-auto opacity-90 md:h-[300px]" />
      <motion.img src="/padel-ball.webp" alt="" aria-hidden="true" variants={{ rest: { scale: 1, y: 0 }, hover: { scale: 1.08, y: -5 } }} transition={springSoft} className="pointer-events-none absolute -bottom-8 -right-2 z-0 h-[160px] w-auto opacity-95 md:h-[180px]" />
      <div className="relative z-10 flex flex-col items-start">
        <span className="type-eyebrow text-white/65">{rate.eyebrow ?? 'Специальное предложение'}</span>
        <h3 className="type-title-card mt-3 text-white leading-tight">{rate.title}</h3>
        <p className="type-body-sm mt-2 text-white/75">{rate.description}</p>
        <ContentAction action={rate.action} sourcePage={sourcePage} sourceEntity={sourceEntity ?? rate.title} variant="primary" size="md" icon={<ArrowRight size={17} />} className="mt-5">
          {rate.action.label || 'Записаться на пробную'}
        </ContentAction>
      </div>
    </MeshCard>
  }
