...
export function RentalRateCard({ rate, sourceEntity, sourcePage, layout = 'compact' }: { rate: RentalRate; sourceEntity?: string; sourcePage?: string; layout?: 'wide' | 'compact' }) {
  if (rate.cardVariant === 'trial' && layout === 'wide') {
    return <MeshCard reveal={false} tone={(rate.meshTone ?? 'deep-blue') as MeshTone} className="relative flex h-full min-h-[240px] flex-col overflow-hidden p-7 text-white md:p-8">
      <ProgressiveImage skeleton={false} src="/padel-racket.webp" alt="" aria-hidden="true" loading="lazy" decoding="async" variants={{ rest: { scale: 1, rotate: 7, y: 0 }, hover: { scale: 1.05, rotate: 3, y: -6 } }} transition={springSoft} className="pointer-events-none absolute -bottom-14 right-[8%] z-0 h-[340px] w-auto opacity-95" />
      <ProgressiveImage skeleton={false} src="/padel-ball.webp" alt="" aria-hidden="true" loading="lazy" decoding="async" variants={{ rest: { scale: 1, y: 0 }, hover: { scale: 1.08, y: -5 } }} transition={springSoft} className="pointer-events-none absolute -bottom-8 -right-4 z-0 h-[210px] w-auto opacity-95" />
  ...
  if (rate.cardVariant === 'trial') {
    return <MeshCard reveal={false} tone={(rate.meshTone ?? 'deep-blue') as MeshTone} className="training-trial-compact relative flex h-full min-h-[500px] flex-col overflow-hidden p-6 text-white md:p-7">
      <ProgressiveImage skeleton={false} src="/padel-ball.webp" alt="" aria-hidden="true" loading="lazy" decoding="async" variants={{ rest: { scale: 1, y: 0 }, hover: { scale: 1.08, y: -7 } }} transition={springSoft} className="pointer-events-none absolute bottom-20 -right-36 z-0 h-[260px] w-auto opacity-95 md:h-[280px]" />
