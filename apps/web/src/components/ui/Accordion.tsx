...
function AccordionItem({
  ...
  const { ref, controls } = useSsrReveal<HTMLDivElement>("-50px");
  ...
  return (
    <div className="flex flex-col">
      {items.map((item, i) => {
        ...
        return <AccordionItem key={`${item.q}-${i}`} item={item} index={i} isOpen={isOpen} triggerId={triggerId} panelId={panelId} onToggle={() => setOpen(isOpen ? null : i)} />;
      })}
    </div>
  );
}
