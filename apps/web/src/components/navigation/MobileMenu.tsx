...
export function MobileMenu({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <BottomSheet open={open} onClose={onClose} title={site.mobileActions.menuTitle}>
    ...
    <div className="se-2 type-body-sm flex flex-col gap-3 bg-surface-subtle p-4 text-ink-soft">
      ...
      <a href={`tel:${site.contacts.phoneValue}`} data-contact-confirmed data-analytics-action="phone" onClick={(event) => { event.preventDefault(); onClose(); window.setTimeout(() => requestContact('phone'), 300) }} className="flex items-center gap-2.5"><PhoneIcon size={16} className="shrink-0" /> {site.contacts.phoneDisplay}</a>
