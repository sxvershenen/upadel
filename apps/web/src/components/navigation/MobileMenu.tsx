...
export function MobileMenu({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <BottomSheet open={open} onClose={onClose} title={site.mobileActions.menuTitle}>
    ...
    <div className="se-2 type-body-sm flex flex-col gap-3 bg-surface-subtle p-4 text-ink-soft">
      ...
      <a href={`tel:${site.contacts.phoneValue}`} data-contact-confirmed data-analytics-action="phone" onClick={(event) => { event.preventDefault(); onClose(); window.setTimeout(() => requestContact('phone'), 300) }} className="flex items-center gap-2.5"><Phone size={16} className="shrink-0" /> {site.contacts.phoneDisplay}</a>
      ...
      {telegram && <a href={telegram.url} data-contact-confirmed data-analytics-action="telegram" onClick={(event) => { event.preventDefault(); onClose(); window.setTimeout(() => requestContact('telegram'), 300) }} target="_blank" rel="noreferrer" className="flex items-center gap-2.5"><Send size={16} className="shrink-0" /> {telegram.label}</a>}
    </div>
