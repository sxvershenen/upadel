...
test('training swipers keep the mobile gutter and hint separate from Swiper state', () => {
  ...
  assert.doesNotMatch(hintSource, /pointerdown/)
  assert.match(hintSource, /addEventListener\("animationend", finish\)/)
  ...
  assert.match(coachSource, /useImageParallax\(photoViewportRef, 11\)/)
  assert.match(coachSource, /data-parallax-viewport/)
  assert.match(coachSource, /data-parallax-layer style=\{\{ y: photoY \}\}/)
