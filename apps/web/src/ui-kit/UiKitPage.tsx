    ...
    <main className="container-page">
      ...
      <KitSection id="surfaces" eyebrow="Surfaces" title="Поверхности и художественные слои" note="Mesh и overlays — разрешённые выразительные исключения. Фирменный spring lift включён по умолчанию; interactive=false оставляет служебную поверхность неподвижной.">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          ...
          {overlayTones.map((overlay) => <div key={overlay} className={`se-3 ${overlay} relative isolate min-h-[150px] overflow-hidden bg-cover bg-center`} style={{ backgroundImage: `url(${home.offers[1]?.image?.url ?? ""})` }}><div className="relative z-10 flex h-full min-h-[150px] items-end p-5 text-white"><Typography role="caption" tone="inverse" className="font-semibold uppercase">{overlay}</Typography></div></div>)}
        </div>
      </KitSection>
