...
export async function createThematicPageProjection(payload: Payload, options: { kind: ThematicPageKind; origin: string; preview: boolean }): Promise<ThematicPageDTO> {
  ...
  if (kind === 'training') {
    ...
    return parseThematicPageDTO({
      ...
      programsEyebrow: String(page.programsEyebrow ?? ''),
      programsTitle: String(page.programsTitle ?? ''),
      blocks: (page.blocks ?? []).map(({ title, body, icon }: Row) => ({ title, body, icon })),
      coachesEyebrow: String(page.coachesEyebrow ?? ''),
      coachesTitle: String(page.coachesTitle ?? ''),
      coachesDesktopActionLabel: String(page.coachesDesktopActionLabel ?? ''),
      coachesMobileActionLabel: String(page.coachesMobileActionLabel ?? ''),
      knowledgeEyebrow: String(page.knowledgeEyebrow ?? ''),
      knowledgeTitle: String(page.knowledgeTitle ?? ''),
      firstVisitTitle: String(page.firstVisitTitle ?? ''),
      firstVisitCopy: String(page.firstVisitCopy ?? ''),
      firstVisitItems: (page.firstVisitItems ?? []).map(({ title, body, icon }: Row) => ({ title: String(title ?? ''), body: String(body ?? ''), icon })),
      faqTitle: String(page.faqTitle ?? ''),
      faqCopy: String(page.faqCopy ?? ''),
      faq: (page.faq ?? []).map(({ question, answer }: Row) => ({ question: String(question ?? ''), answer: String(answer ?? '') })),
