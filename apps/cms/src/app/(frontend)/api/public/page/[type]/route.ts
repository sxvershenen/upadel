...
export async function GET(request: Request, context: { params: Promise<{ type: string }> }): Promise<Response> {
  ...
  if (!kinds.has(type as ThematicPageKind)) return Response.json({ error: 'Unknown page type.' }, { status: 404, headers: { 'Cache-Control': 'no-store' } })
  ...
  try {
    ...
    const result = await createThematicPageProjection(payload, { kind: type as ThematicPageKind, origin: url.origin, preview })
