...
Public lead forms use the shared `Field`, `TextareaField`, `SelectField` and `CheckboxField` atoms with accessible screen-reader labels; dialogs keep their header fixed while only the content viewport scrolls. Simple reveal entrances remain a lightweight SSR-safe DOM/IntersectionObserver enhancement rather than forcing every static atom into a React island or GSAP bundle.
