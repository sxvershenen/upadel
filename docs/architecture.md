...
The shared Astro layout adds intent-based prefetching for internal links and a DOM text typographer for non-breaking short Russian words, while preserving semantic HTML and explicit opt-out selectors for code-like content. Above-the-fold page and mobile-chrome entrances, plus the homepage hero's initial parallax transform, are emitted with SSR markup and CSS so they start before React hydration; interactive scroll and layout motion remains island-owned. Its CMS-backed desktop header and mobile bottom navigation share one persistent React island outside the Swup replacement surface, so fixed chrome and menu state survive page navigation. React alone owns desktop compact state; the first client render matches the expanded SSR header, then a layout effect applies the responsive icon mode before paint. Icon/label visibility and vertical geometry stay deterministic through hydration. Public content images use one reversible progressive-image atom: the image element itself renders a black animated skeleton within its own dimensions/radius, then reveals the decoded image without changing layout; logos and transparent decorative assets opt out. No React state update is performed on image completion. CMS rich-text images use the same data contract without another island. LAN development may project media/forms through the Astro same-origin proxy so phones need only the web port; production keeps the CMS origin directly. The optional Swup/GSAP/Three transition runtime loads only after intent on an internal link; the tiny audio module is warmed at the same point and never blocks the transition. Reveal runtime starts from document readiness and the live Swup surface; SSR content stays visible until React commits, then only offscreen reveal targets are prepared. Page-owned CSS entrances suppress nested reveals. Swup synchronizes route stylesheets and SEO head tags before content replacement, retains loaded styles, and lets Astro hydrate incoming islands.
Public lead forms use the shared `Field`, `TextareaField`, `SelectField` and `CheckboxField` atoms with accessible screen-reader labels; dialogs keep their header fixed while only the content viewport scrolls. Reusable surfaces and `Reveal` emit GSAP reveal metadata directly from the component layer, while `Typography`, `Badge`, `Button` and `ButtonLink` expose an explicit opt-in reveal contract for atomic entrances. Cards are reveal boundaries so their children never start competing animations. One lazy, SSR-safe DOM controller loads GSAP directly after React commits and uses IntersectionObserver plus mutation refreshes to start only visible animations, without forcing each atom into a separate React island. Hero CTAs use CSS entrances only, and hero parallax selects the visible mobile asset with a desktop fallback. Initial GSAP targets remain visible in SSR; offscreen targets gain scroll reveals after hydration, while a fast-scroll late start settles already visible targets without blanking them. Default reveals have no page-global stagger; only explicit local delays are honored. Repeated readiness signals for the same surface do not restart the controller. Whole detail-page containers use opacity-only CSS entrances so native scroll restoration cannot accumulate a transform offset; tournament tab transitions run only after a selection change. GSAP reveals target only `#swup`; completion, interruption and restart restore pre-animation inline styles and release reveal ownership. Transition waits preserve cleanup callbacks and settle on completion or interruption; canceled visits resume the current page's reveal observers.
...
Migration stage: the CMS-backed homepage server-renders visible HTML plus one `client:load` React island. Published CMS edits are read on the next request without rebuilding the public route; reveal animations never hide SSR content. The optional Swup/GSAP/Three transition runtime loads only after internal-link intent, with audio warmed in parallel. Dedicated `/preview/*` routes use the Node adapter for secure live drafts. Split the island later, guided by measured runtime cost and visual regression checks.
