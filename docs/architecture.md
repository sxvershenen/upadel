...
The transition demo uses the attached reference project's native WebGL2 scene: a client-only canvas renders its procedural shader ball, 3D bezier position, perspective depth, spin and particle trail over a fixed transition timeline. Reference start/peak/end anchors and timing are retained, while a narrow Hermite correction around the peak keeps world- and screen-space velocity continuous. Mobile uses a portrait-rotated trajectory, reduced geometry detail and a capped pixel ratio to avoid frame stalls; its material is pre-baked once and its backing resolution steps down only after sustained slow frames. Steady-state frames update only model and normal matrices, with uniform locations and static shader inputs cached. Swup still replaces only the SSR content container. Rendering stops and clears the transparent canvas when no flight is active, and navigation remains functional when WebGL2 is unavailable.
